...
export async function reserverLots(
  ...
  const { error: collectError } = await admin.from("collect").insert(
    entries.map(({ lot, code }) => ({
      id_lot: lot.id_lot,
  ...
  const assocLotsHtml = entries
    ...
    <tr style="background:#fff;">
      <td style="padding:2px 14px;color:#374151;font-size:13px;"><strong>Adresse de récupération :</strong> ${esc(lot.adresse_recup)}</td>
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        ...
        <div style="background:#f9fafb;padding:24px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb;border-top:none;">
          ...
          <p style="color:#374151;">Votre réservation de <strong>${entries.length} lot(s)</strong> a bien été enregistrée.</p>
          <div style="background:rgba(6,87,63,0.08);border:1px solid rgba(6,87,63,0.2);border-radius:8px;padding:16px;margin:16px 0;">
            <p style="margin:0;font-weight:bold;color:#06573f;">Créneau de récupération</p>
          ...
          <p style="color:#6b7280;font-size:13px;">Les commerçants concernés ont également reçu une notification.</p>
  ...
  const byCommercant = entries.reduce(
    (acc, { lot, code }) => {
      const key = lot.id_commercant;
      if (!acc[key]) acc[key] = [];
  ...
  await Promise.all(
    Object.values(byCommercant).map(async (items) => {
      const commercant = commercantMap.get(items[0].lot.id_commercant);
